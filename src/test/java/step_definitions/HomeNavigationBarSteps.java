...
public class HomeNavigationBarSteps implements CommonPage {
    ...
    @When("User is click 0n {string} navigation bar")
    public void user_is_click_0n_navigation_bar(String navigationBar) {
        WebDriverManager.click(By.linkText(String.format(XPATH_TEMPLATE_NAVIGATION_BAR_BUTTON, navigationBar)));
    }

    @Then("Verify {string} is displayed")
    public void verify_is_displayed(String topBarMenu) {
        Assert.assertTrue(WebDriverManager.isDisplayed(By.xpath(String.format(XPATH_TEMPLATE_TOP_BAR_MENU, topBarMenu))));
    }

    @When("Click on English button for language Selection")
    public void click_on_english_button_for_language_selection() {
        WebDriverManager.click(By.xpath(String.format(XPATH_TEMPLATE_LANGUAGE_BUTTON)));
    }

    @Then("Verify {string} Selection is displayed")
    public void verify_selection_is_displayed(String languageSelection) {
